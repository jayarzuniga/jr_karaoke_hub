...
if (!channelUrl) {
  throw new Error('Usage: node scripts/remove-channel-songs.mjs <youtube-channel-videos-url>')
}
...
const channelVideoIds = new Set(scrapedVideos.map((video) => video.videoId))
const channelIdentities = new Set(
  scrapedVideos.map((video) => {
    const derived = deriveSongFields(video.title)
    return `${normalizeIdentity(derived.title)}__${normalizeIdentity(derived.artist)}`
  }),
)

const keptSongs = existingRawSongs.filter((song) => {
  ...
  return !channelVideoIds.has(youtubeId) && !channelIdentities.has(identity)
})

const removedSongs = existingRawSongs.length - keptSongs.length

if (!removedSongs) {
  console.log(JSON.stringify({ scraped: scrapedVideos.length, removed: 0 }, null, 2))
  process.exit(0)
}

const serializedSongs = keptSongs.map(formatRawSong).join('\n')
...
if (updatedSongsFile === songsFile) {
  throw new Error('Could not write filtered songs back into songs.ts')
}
